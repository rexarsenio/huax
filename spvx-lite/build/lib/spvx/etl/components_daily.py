...
def _load_dwell_daily(con: duckdb.DuckDBPyConnection) -> Tuple[Dict[Tuple[str, dt.date], float], Dict[Tuple[str, dt.date], int]]:
    df = con.execute(
        """
        SELECT
...
def build_components_daily(con: duckdb.DuckDBPyConnection) -> int:
    ...
    component_rows: List[Tuple[dt.date, str, str, Optional[float], Optional[float], Optional[int], Optional[str], int]] = []

    for date in all_dates:
        doy = dt.date.fromisoformat(str(date)).timetuple().tm_yday if isinstance(date, str) else date.timetuple().tm_yday
        ...
        component_rows.append(
            _build_component_row(date, "APAC", "CQ_SG", sg_raw, sg_obs, sg_z, sg_reason, sg_flag)
        )
        ...
        component_rows.append(
            _build_component_row(date, "NAM", "CQ_PAN", pan_raw_total, pan_obs_total, pan_z, pan_reason, max(pan_n_flag, pan_s_flag))
        )
        ...
        component_rows.append(
            _build_component_row(date, "NAM", "PORT_US", port_us_raw, port_us_count, port_us_z, port_us_reason, max(pan_n_flag, pan_s_flag))
        )
        ...
        component_rows.append(
            _build_component_row(date, "SAM", "PORT_BR", port_br_raw, port_br_count, port_br_z, port_br_reason, pan_s_flag)
        )
        sam_flag = pan_s_flag
