...
def _serialise_index_frame(df: pd.DataFrame, value_column: str, extra_columns: Sequence[str] | None = None) -> list[dict[str, Any]]:
        ...
        series.append(
            {
                ...
                "weather_flag": int(row.weather_flag) if hasattr(row, "weather_flag") and row.weather_flag is not None else 0,
            }
        )
...
def ops_health():
    ...
    if not components_df.empty:
        ...
        observed_components = {
            basin: sorted(subset["comp"].unique())
            for basin, subset in components_df.groupby("basin")
        }
