...
def train_models():
    ...
    aucs = ", ".join(
        f"{h * 24}h={throughput_metrics.auc_by_horizon[h]:.3f}" for h in throughput_metrics.auc_by_horizon
    )
    rprint(
...
@app.command("score")
def score_models(simulate_degradation: bool = typer.Option(False, help="Simulate a degraded run (missed fix scenario).")):
    ...
    rprint(f"[green]Scores written to {spread_path} and {throughput_path}[/green]")
...
def ingest_sea_state(
    provider: str = typer.Option("auto", help="auto | rtofs | cmems. 'auto' uses fallback chain: CMEMS → RTOFS → degraded"),
    ...
    if provider == "auto":
        ...
        for cid in chokepoint_ids:
            ...
            with log_step(f"Processing {cid}"):
                ...
                # Update metrics
                try:
                    from spvx.metrics import spvx_sea_state_records_total, spvx_sea_state_freshness_seconds
    ...
    *,
    ...
    if throughput_path.exists():
        ...
        if not throughput_df.empty:
            ...
            abs_err = np.abs(
                throughput_df["throughput_pred_72h"] - throughput_df["throughput_actual_72h"]
            ).dropna()
            payload["throughput_72h"] = {
                "mae_units": float(abs_err.mean()) if not abs_err.empty else None,
                "nowcast": {
                    str(pd.to_datetime(latest["date"]).date()): float(latest["throughput_pred_72h"])
                },
            }
