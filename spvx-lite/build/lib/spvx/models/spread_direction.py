...
def _calibration_bins(y_true: np.ndarray, y_pred: np.ndarray, n_bins: int = CALIBRATION_BINS) -> List[Tuple[float, float]]:
    prob_true, prob_pred = calibration_curve(y_true, y_pred, n_bins=n_bins, strategy="quantile")
    return list(zip(prob_pred.tolist(), prob_true.tolist()))
...
def train_spread(model_path: str | Path = "data/outputs/model_spread.pkl") -> TrainResult:
    X, y, spread_fwd, artefacts = _prepare_dataset()
    splitter = _time_series_split(len(X))

    fold_metrics: List[Dict[str, float]] = []
    cv_predictions = np.full(len(X), np.nan, dtype=float)
    valid_mask = np.zeros(len(X), dtype=bool)

    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(X)):
        pipeline = _make_pipeline()
        pipeline.fit(X.iloc[train_idx], y.iloc[train_idx])
        preds = pipeline.predict_proba(X.iloc[test_idx])[:, 1]
        cv_predictions[test_idx] = preds
        valid_mask[test_idx] = True
        metrics = _fold_metrics(y.iloc[test_idx].to_numpy(), preds)
    ...
    y_valid = y.iloc[valid_mask].to_numpy()
    sign_flip_auc = (
        roc_auc_score(y_valid, -cv_preds_valid) if len(np.unique(y_valid)) > 1 else float("nan")
    )
...
def score_spread(model_path: str | Path = "data/outputs/model_spread.pkl") -> ScoreResult:
    ...
    features, y, spread_fwd, artefacts = _prepare_dataset()
    ...
    scores = pd.DataFrame(
        {
            ...
            "label": y.values,
            "spread_fwd": spread_fwd.reindex(X.index).values,
        }
    ).sort_values("date")
