...
def _list_days_yyyymmdd(lookback_days: int) -> list[str]:
    ...
    return [
        (today - dt.timedelta(days=k)).strftime("%Y%m%d")
...
def _open_dataset(nc_files: list[str]) -> Optional[xr.Dataset]:
    if not nc_files:
        return None
    try:
...
def _clip_dataset(ds: xr.Dataset, bbox: Dict[str, float]) -> Optional[xr.Dataset]:
...
def _select_first_variable(ds: xr.Dataset, candidates: Iterable[str]) -> Optional[str]:
