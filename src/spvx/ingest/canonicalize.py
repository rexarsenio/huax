...
def _parse_timestamp(payload: Dict[str, Any]) -> Optional[dt.datetime]:
    """
    Attempt to resolve the AIS message timestamp.
    Fallback to current UTC if the payload does not provide a usable value.
    """
    candidates = []
    ...
    # AISStream adds receivedTimestamp in milliseconds since epoch
    candidates.append(metadata.get("receivedTimestamp"))
    candidates.append(metadata.get("timestamp"))
    candidates.append(report.get("UtcSec"))
    candidates.append(report.get("Timestamp"))
    ...
    for value in candidates:
        ...
        try:
            if isinstance(value, (int, float)):
                ...
                # AIS Timestamp field (0-59) -> combine with current minute
                if 0 <= value < 60:
...
def canonicalize(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Normalize the incoming AIS payload.
    Returns a dict that can be inserted into ais_canon or None if the message is invalid.
    """
    message = payload.get("Message") or {}
    report = message.get("PositionReport") or {}
